<template>
        ...
        <v-hover>
            <template class="align-center" v-slot="{ hover }">
                <v-card class="mx-3" :elevation="hover ? 24 : 6">
                    ...
                    <v-card-actions dark>
                        <v-spacer></v-spacer>
                        Salvar<v-btn :disabled = "!Valido" icon @click.stop="dialog = true">
    ...
    <v-dialog v-model="dialog" max-width="290">
        <v-card >
            <v-card-text class="headline py-1 px-2"><b>Desea cambiar los parámetros de conexión?</b></v-card-text>

            <v-card-text>
                Debe recordar la contraseña y volver a conectar su dispositivo móvil o PC, al sistema con sus nuevas características.
            </v-card-text>
...
export default {
    ...
    data() {
        return{
            ...
            rules: {
                ...
                password(v){
                    if((v || '').match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*(_|[^\w])).+$/)){return true;}else{return 'La contraseña debe contener una letra mayúscula, un carácter numérico y un carácter especial'}
                },
                length(len,v){
    ...
    computed: {
        ...
        Valido(){
            ...
            if((this.rules.password(this.password) === true) && (this.rules.length(this.passlen,this.password) === true) && (this.rules.length(this.ssidlen,this.ssid)) === true){
                valido = true;
            }else{valido=false;}
            return(valido)
        }
