...
export function renderChart(wrapper, curData) {
    ...
    const initialCircleFillGap = 0.07
    const waveCount = 1
    const circleColor = '#178BCA'
    const textColor = '#5a98d5'
    const waveColor = '#178BCA'
    const waveTextColor = '#A9DBf8'//'#ffffff'
    const textVertPosition = 0.2
    const waveRiseTime = 2000
    const waveAnimateTime = 1000
    const waveOffset = 0
    const waveHeight = 2
    const fillPercent = Math.max(minValue, Math.min(maxValue, curData)) / maxValue
    const textRounder = (value) => String(parseFloat(value).toFixed(0))
